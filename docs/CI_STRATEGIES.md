...
On `push` to `main`, we detect merges from `changeset-release/*` and `hotfix/*` by looking up the **Pull Request associated with the pushed commit** via the GitHub API, then reading the PR’s **head branch name** (`pr.head.ref`).
