Q: How do I open a modal from TypeScript without a trigger button?

A: See the Usage / Opening programmatically section. Omit `[rteModalTrigger]` and call `showModal()` on the `rteModal` export through `@ViewChild`. Call `close()` from footer buttons when the user confirms or cancels.

Q: Storybook shows the same HTML for several Modal examples — how do I know what differs?

A: See the Usage / Storybook snippets vs. variant differences section. Angular autodocs display template markup; differences between stories are usually directive input bindings (`[rteModalSize]`, `[rteModalTitle]`, `#customContent`, etc.) or TypeScript logic that calls `showModal()`. This documentation states which binding changes for each example.
