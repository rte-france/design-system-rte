The `SegmentedControlComponent` (`<rte-segmented-control>`) is the public API. Pass segment definitions through `options`, control the active segment with `selectedSegment`, and handle selection changes with `change`.

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `options` | `SegmentOptions[]` | `[]` | Segment definitions rendered in the control. Must contain 2 or 3 items. |
| `selectedSegment` | `string` | — | Id of the currently selected segment. Must match an `id` in `options`. |
| `change` | `EventEmitter<string>` | — | Emits the `id` of the segment the user selected. |
...
| `ariaLabel` | `string` | — | Accessible name for the radiogroup when no visible label is associated. |

Each entry in `options` accepts the following fields:

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `id` | `string` | — (required) | Unique identifier for the segment. Emitted by `change`. |
