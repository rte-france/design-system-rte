...
Ellipsis and the tooltip that reveals the full label depend on `breadcrumbItemMaxWidth`. The check runs once after the item renders; labels that change dynamically after mount may not update ellipsis state until the component is re-created.

### Fixed separator

Items are separated by a `/` character. The separator is not configurable through component inputs.
