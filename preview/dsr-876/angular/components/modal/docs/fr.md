...
Importez `ModalModule` et composez un élément hôte avec la directive `[rteModal]`. Attachez `[rteModalTrigger]` à l'élément qui ouvre la modale, et fournissez les actions du pied de page via les templates `#primaryButton` et `#secondaryButton`.
...
Appelez `modalHost.close()` depuis les boutons d'action ou utilisez `modalHost.showModal()` pour ouvrir la modale de manière programmatique.
