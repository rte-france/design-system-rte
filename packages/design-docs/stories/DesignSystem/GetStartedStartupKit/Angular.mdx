        ...
        <Accordion title="Installation" id="installation">
            <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "4px 8px" }}>
                ...
                <DocsSection title="Prérequis et Peer Dependencies" headingLevel={3}>
                    <Paragraph>Ce guide suppose que vous avez déjà une application **Angular** existante. La version d'**Angular** doit être **19** ou supérieure.<br/>Le package utilise aussi les peer dependencies suivantes (qui devraient déjà être présentes dans votre projet) :</Paragraph>
                    <List>
                        <li><code>@angular/common : ^19.0.0</code></li>
                        <li><code>@angular/core : ^19.0.0</code></li>
                        <li><code>sass : ^1.85.0</code></li>
                    </List>
                </DocsSection>

                <DocsSection title="Compatibilité des versions d'Angular supérieures" headingLevel={3}>

                    <Paragraph>Le **Design System Luciole** est compatible avec **Angular 19** et les versions supérieures. Pour les versions supérieures à 19, un contournement est nécessaire lors de l'installation du package : </Paragraph>
                    ```bash
                    npm install -f @design-system-rte/angular
                    ```
                </DocsSection>
            </div>
        </Accordion>
        ...
        <Accordion title="Configuration initiale" id="setup-configuration">
            <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "4px 8px" }}>
                ...
                <DocsSection title="Ajouter les sélecteurs HTML" headingLevel={3}>
                    ...
                    <Paragraph>
                        <code>data-theme</code> est requis sur chaque scope thématisé. Lorsque <code>data-mode</code> est omis,
                        le mode est hérité d'un ancêtre portant <code>data-mode</code>. Si aucun ancêtre ne définit de mode,
                        <code>light</code> est utilisé par défaut.
                    </Paragraph>
                ...
                <DocsSection title="Configuration des icônes" headingLevel={3}>
                    <Paragraph>Les icônes sont intégrées au bundle de la librairie Angular. Aucune copie de fichiers SVG dans <code>/assets/icons</code> n'est requise pour utiliser <code>rte-icon</code>.</Paragraph>
                </DocsSection>
            </div>
        </Accordion>
        ...
        <Accordion title="Thématisation et personnalisation" id="theming-customization">
            <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "4px 8px" }}>
                ...
                <DocsSection title="Changer de thème" headingLevel={3}>
                    ...
                    <Paragraph>
                        Pour appliquer un thème local à une section de l'application, il suffit de définir{" "}
                        <code>data-theme</code> sur l'élément conteneur. Le mode est hérité du parent le plus proche
                        portant <code>data-mode</code> (généralement <code>&lt;body&gt;</code> ou <code>&lt;html&gt;</code>).
                    </Paragraph>
                    ...
                    <body data-theme="bleu_iceberg" data-mode="light">
                      ...
                      <div data-theme="violet">
                        <!-- Composants RTE en thème violet, mode light hérité -->
                      </div>
                    </body>
                    ```

                    <Paragraph>
                        Pour forcer un mode localement (par exemple en mode sombre), définissez explicitement les deux
                        attributs sur le même élément : <code>data-theme="violet" data-mode="dark"</code>.
                    </Paragraph>
