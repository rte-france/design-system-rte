        ...
        <Accordion title="Installation" id="installation">
            <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "4px 8px" }}>
                ...
                <DocsSection title="Compatibilité des versions d'Angular supérieures" headingLevel={3}>
                    <Paragraph>Le package <code>@design-system-rte/angular</code> déclare des <code>peerDependencies</code> restreintes à une version majeure d'**Angular** (<code>^18</code>; <code>^19</code>; etc).
                    Cette contrainte ne signifie pas que le **Design System** est incompatible avec des versions plus récentes d'**Angular**, mais que nous n'avons pas encore validé son fonctionnement sur ces versions par des tests. Tant que cette validation n'est pas faite, nous préférons ne pas l'annoncer comme officiellement supporté.</Paragraph>
                    <Paragraph>Si vous installez le package sur un projet utilisant une version d'**Angular** supérieure, **npm** peut refuser l'installation avec une erreur <code>ERESOLVE</code> :</Paragraph>
                    
                    ```bash
                    npm error ERESOLVE unable to resolve dependency tree
                    ```

                    <Paragraph>Dans ce cas, vous pouvez forcer l'installation avec l'option <code>--force</code> ou <code>-f</code> :</Paragraph>
                    ```bash
                    npm install -f @design-system-rte/angular
                    ```
                    <Paragraph>En pratique, le **Design System** fonctionne correctement avec les versions plus récentes d'**Angular**. Cette restriction est une mesure de prudence de notre part, le temps d'élargir et de documenter officiellement la compatibilité avec les nouvelles versions majeures.</Paragraph>
                </DocsSection>
            </div>
        </Accordion>
