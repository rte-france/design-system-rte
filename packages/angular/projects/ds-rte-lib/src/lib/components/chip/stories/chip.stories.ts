  ...
  render: (args) => {
    ...
    return {
      props: {
        ...
        options,
        selectedChip: null,
        onChipClick(event: Event, optionId: string) {
          console.log("Chip clicked:", optionId);
          console.log(selectedChip);
          if (selectedChip === optionId) {
            selectedChip = null;
          } else {
            selectedChip = optionId;
          }
        },
      },
      inputs: ["selectedChip"],
        ...
        <div style="display: flex; gap: 10px;" role="radiogroup">
          <rte-chip
            *ngFor="let option of options"
            ...
            (click)="onChipClick($event, option.id)"
            class="chip"
          ></rte-chip>
        </div>
        <p>Chip sélectionnée: {{${selectedChip}}}</p>
      `,
    };
  },
};
