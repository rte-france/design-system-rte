...
Contrôlez `Modal` avec `isOpen` et `onClose`. Définissez `isOpen` à `true` depuis n'importe quel gestionnaire d'événement ou callback asynchrone, et connectez les actions du pied de page via `primaryButton` et `secondaryButton`.
...
Vous n'avez pas besoin d'un bouton « Open modal » dédié. Appelez `setIsOpen(true)` depuis la logique du composant — par exemple après un échec de validation ou lorsqu'une tâche en arrière-plan se termine.
...
Comme `isOpen`, `size` et `children` sont des props JSX, les snippets Storybook tendent à rendre les différences entre variantes plus visibles que les snippets template-only Angular.
...
Seule la prop `size` change entre les instances de modale. Les props partagées (`title`, `description`, `icon`, boutons, etc.) restent identiques.
...
Par rapport à la configuration par défaut, cet exemple définit `size="m"` et passe des champs de formulaire en `children`.
...
Q : Comment ouvrir une modale depuis le code sans bouton « Open modal » ?

R : Consultez la section Usage / Ouverture programmatique. Définissez `isOpen` à `true` depuis n'importe quel gestionnaire d'événement ou callback asynchrone. Un bouton déclencheur dédié est optionnel.
