Q: How do I open a modal from code without an "Open modal" button?

A: See the Usage / Opening programmatically section. Set `isOpen` to `true` from any event handler or async callback. A dedicated trigger button is optional.
