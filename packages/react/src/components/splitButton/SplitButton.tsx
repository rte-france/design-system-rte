...
const SplitButton = forwardRef<HTMLElement | HTMLButtonElement, SplitButtonProps>(
  (
    ...
    const hasBadge = options.some((option) => option.showBadge);

    const badgeCount = hasBadge
      ? options.reduce((acc, option) => acc + (option.badgeCount ? option.badgeCount : 0), 0)
      : undefined;
    ...
    return (
      ...
      >
        ...
        <Dropdown
          ...
          trigger={
            hasBadge ? (
              <Badge count={badgeCount} content="number" badgeType="indicator">
                <button
                  type="button"
                  aria-haspopup="menu"
