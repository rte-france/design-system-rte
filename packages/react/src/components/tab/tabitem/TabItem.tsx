          ...
          setHoverIndicatorStyle((prev) => ({
            ...
            left:
              tabItemRef.current?.offsetLeft !== undefined
                ? tabItemRef.current.offsetLeft - 2 - (tabItemRef.current.parentElement?.scrollLeft ?? 0)
                : undefined,
    ...
    useEffect(() => {
      if (tabItemRef.current) {
        if (direction === "horizontal") {
          ...
          setHoverIndicatorStyle({
            left: tabItemRef.current.offsetLeft - 2 - (tabItemRef.current.parentElement?.scrollLeft ?? 0),
            top: tabItemRef.current.offsetTop,
            height: tabItemRef.current.offsetHeight,
            opacity: 0,
          });
