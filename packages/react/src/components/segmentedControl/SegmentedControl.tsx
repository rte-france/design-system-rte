...
const SegmentedControl = forwardRef<HTMLDivElement, SegmentedControlProps>(
  ({ options, onClick, selected, ...props }, ref) => {
    ...
    useEffect(() => {
      ...
      const idx = options.findIndex((opt) => opt.id === selected);
...
const Segment = ({ id, icon, label, position, selected, onClick, parentRef }: SegmentProps) => {
    ...
    <div ref={ref} role="radio" className={style["segment-container"]} data-position={position}>
      ...
      >
        ...
        <div className={style["segment-content"]}>
          {icon && <Icon name={icon} appearance="filled" size={24} />}
