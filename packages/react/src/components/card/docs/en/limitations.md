...
When `clickable` is `true`, click events on nested interactive elements bubble to the card and trigger `onClick`. Call `stopPropagation()` on nested handlers when the inner control should not activate the card.
